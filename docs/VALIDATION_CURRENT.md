...
更新日期：2026-09-22。源版本见 [VERSION](../VERSION)。本页区分源码回归与可执行文件验证；历史报告不自动代表当前发布产物。

## 源码基线

第七轮提交恢复实现：[协议与行为报告](./SUBMISSION_RELIABILITY_ROUND_7.md)。当前后端全套 **370 passed、1 skipped**（含本地 Blender/IFC 集成），Node **51 passed**，浏览器 **54 passed**，类型、ESLint、静态构建与 7 份文档检查通过。路由契约为 102 个路径/方法条目。跳过项仍是缺少操作员彩膜素材。新增测试不调用付费模型；断响应浏览器用例连接真实隔离提交 API 和离线 worker。
