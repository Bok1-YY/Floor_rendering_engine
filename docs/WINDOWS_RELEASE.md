...
4. 构建器使用 git archive 固定 HEAD，安装独立构建依赖，在快照内 npm ci/build，编译最多使用 4 个任务。输出位置见工作目录的 latest-build.json。

发布程序包含前端、必要本地图像资源、IfcOpenShell 及 Python 运行依赖。Blender 为外部依赖。Blender 单独解释器所需的源码存入打包资源 ZIP，在程序的临时资源目录中释放；只允许预期的 Python 文件路径。构建版本元数据不再使用固定 7.1.0 或虚构公司名。
