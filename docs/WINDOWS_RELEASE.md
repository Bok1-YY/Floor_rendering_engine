...
固定版本 MinGW 的头文件搜索问题由编译器预检处理：仅从该工具链复制对应头文件到本轮 include 覆盖目录，记录其 SHA-256；不改系统 SDK。应用自身模块保持 C 编译；第三方 Python 模块以 Nuitka 内置字节码分发（清单记入构建报告），保留其原生扩展。该方式避免大型 SWIG 绑定的 C 编译内存溢出，并减少无必要的依赖重编译；PDF、IFC 和图像库须经实际运行验证。
