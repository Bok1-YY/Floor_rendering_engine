...
2. 检查远端标签和 Release，选择未使用的版本。第七轮目标 7.1.2，冲突时顺延补丁号，不覆盖旧版。
3. 执行 build_windows.bat --non-interactive；也可添加 --work-dir 指定本轮专属工作目录。已有目录必须包含构建器识别的 ownership.json，不使用未标记目录。
4. 构建器使用 git archive 固定 HEAD，安装独立构建依赖，在快照内 npm ci/build；pip、npm 及编译缓存均位于本轮工作目录，避免旧全局缓存权限影响构建。编译使用 2 个任务及低内存模式，并先运行小型编译器预检。输出位置见工作目录的 latest-build.json。
...
第七轮另验提交实例跨重启稳定、已受理凭据重放、清卡后的不可用提示、准备阶段查询、实例冲突和损坏凭据。正式 exe 只用隔离凭据/队列样本执行无付费重放，不添加测试开关。首次受理与断响应通过源码浏览器连接真实隔离后端及离线 worker 验证；报告分别标注这两类证据。
