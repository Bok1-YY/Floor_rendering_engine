...
from tools.goal_loop_v2.op003_adjudication_candidate import build_op003_geometry_adjudication_candidate,validate_op003_geometry_adjudication_candidate
...
def test_op003_packet_preserves_pair_and_zero_jamb_blocker():
    d,s,w=_inputs();c=build_op003_geometry_adjudication_candidate(d,EVIDENCE,s,w)
...
def test_op003_packet_rejects_rehashed_pair_input_and_promotion():
    import tools.goal_loop_v2.op003_adjudication_candidate as module
    d,s,w=_inputs();c=build_op003_geometry_adjudication_candidate(d,EVIDENCE,s,w);f=deepcopy(c);f['room_pair_candidate']=['wc','kitchen'];f['candidate_hash']=module._hash({k:v for k,v in f.items() if k!='candidate_hash'})
    with pytest.raises(ValueError,match='geometry evidence drift'):validate_op003_geometry_adjudication_candidate(d,EVIDENCE,s,w,f)
    ...
    with pytest.raises(ValueError):build_op003_geometry_adjudication_candidate(d,EVIDENCE,bad,w)
    p=deepcopy(c);p['source_confirmation']=True
    with pytest.raises(ValueError,match='promoted'):validate_op003_geometry_adjudication_candidate(d,EVIDENCE,s,w,p)
