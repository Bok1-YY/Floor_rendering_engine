...
def test_op002_wrapper_fields_and_hash():
 d=json.loads((ROOT/'data/goal_loop_v2/references/1308/reference-coordinate-authorized-v21.json').read_text()); c=build(d); assert c['opening_id']=='OP002'; assert c['packet']['directed_side_assignment']=={'side_a':'bedroom_corridor','side_b':'bedroom_01'}; assert c['packet']['jamb_support_m']['minimum_jamb_m']>=minimum_jamb_support_m(d); assert validate(d,c)==c
