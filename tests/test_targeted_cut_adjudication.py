...
def test_targeted_triplet_is_stable_distinct_and_unpromoted():
    main()
    d=json.loads((ROOT/'reports/targeted_cut_adjudication_20260902/targeted-cut-adjudication.json').read_text())
    assert [x['opening_id'] for x in d['openings']]==['OP003','OP004','OP009']
    ...
    assert all(len(a['sha256'])==64 for x in d['openings'] for a in x['artifacts'].values())
    for row in d['openings']:
        assert [face['space_ids'][0] for face in row['pre_cut_faces']]==row['merged_group']
