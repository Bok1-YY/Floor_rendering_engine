...
- **全屋设计取代全屋自动 3D / 360° VR**：上传户型图片或 PDF，先确认轻量房间/入口/厨卫摘要，再以自由需求和可选材料参考生成两张 2K 正俯视 2.5D 草稿；选择结构通过的草稿后精修一张 4K 成稿。自动 QA 发现结构硬错误时不能人工覆写，锁定方案可导出带原户型、设计图、提示词快照、QA 和验收合同的 Blender Agent 任务包。输出明确是营销概念图，不是 CAD、BIM 或施工图。
...
- 当前回归集覆盖提示词黄金样本、路由契约、路径安全、队列恢复、电影规划、全屋设计 revision/结构锁/建模包、地板占比、校色、独立样品对色、智能选区和记录链路；本次发布为 **244 tests passed，1 skipped**。
