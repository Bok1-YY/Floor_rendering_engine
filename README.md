...
- **记录与导出**：历史记录持久化、收藏、导出 HTML / PPTX 提案 deck。
- **用量统计**：出图数量、成功率、明细一览。
...
- **引擎模块**是 headless 的核心逻辑（提示词、模型调用、识色、配方、记录、导出），可独立复用。
...
5. 对结果可**磨缝 / 二次编辑 / 重抽**；满意的可**收藏**。
6. 到「**记录**」页查看历史、导出 HTML / PPTX 提案；「**用量**」页看统计。
...
├── records.py                  # 记录持久化、收藏、导出 HTML/PPTX
