...
Floor Rendering Engine 现在是一套本地 AI 空间设计与视觉生产工作台，包含两条彼此独立、又能衔接的核心产品线：

- **地板产品视觉生产**：从真实色板、场景提示词和多模型生成，到颜色锁定、局部修补、评审、记录与交付。
- **全屋设计研究快线**：从原始户型图、人工比例尺和关键锚点出发，经 Gemini 九问结构审查与严格几何合同，生成可继续编辑和验证的 Blender、GLB 与研究 IFC 建筑灰模。

它不是给图片套一个“家居效果图”模板，也不是把 AI 生成的漂亮俯视图冒充施工 BIM。系统同时关心产品色是否准确、空间是否可信、墙门窗是否可追溯、模型失败后怎么修，以及结果能否进入下一段真实生产流程。

> 从产品色到空间结构，让 AI 结果经过确认、验证和交付，而不只是多生成几张图。
