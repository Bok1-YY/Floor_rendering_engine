...
- **双轨 360° VR 与本地几何/地板校准**：纯效果图可扩展为 3840×1920 ERP，地板小样也可走 B2 / GPT Image 2 六面图集直出路线；直出路线可额外上传一张空间参考效果图作为正前方几何锚点。两条路线在模型返回后都由本地几何合同接管：OpenCV 标定 FOV、地平线和 Manhattan 方向，八个标准透视视角检查墙体直线；CLIPSeg 判断“是不是地板”，MobileSAM 贴合家具边缘，Depth Anything V2 Small 审核遮挡边界，三者均在本机运行。上传原厂整体彩膜时，系统按毫米宽度、长边周期、实际板宽和切板相位直接采样原像素，不让生图模型重新生成木纹；所有方向共享一个球面射线—水平地板世界坐标，脚下由 nadir 多点识别而不是固定像素带强填。低置信度结果保留为草稿，可在结果卡或历史记录中打开“本地几何/地板校准”免费修改五向蒙版、相机高度、铺装方向与尺度，不会再次调用 Fal/Gemini。
- **DWG / DXF 直接进入整屋灰模**：内置基于 MIT 许可 ACadSharp 的本地 DWG 读取适配器，可直接读取 AutoCAD 2004 等 DWG 并转换为 ASCII DXF；ezdxf + Shapely 随后完成单位、图层、动态块、墙线、门窗和家具 INSERT 的可追溯解析。多套平面不会盲选，页面提供候选缩略图供明确选择；Gemini 只补房间名称与布局语义，CAD 墙线、尺寸和 INSERT 变换由本地事实哈希冻结。解析、候选选择和几何门禁均发生在任何生图调用之前。
- **整屋 3D 统一建模取代单房间猜图**：户型识别采用“整屋初提取 → 彩色叠图拓扑复核 → 门窗专项复核”三阶段 Gemini 分工，再由本地检查外墙闭环、封闭房间缺墙、墙体两侧房间关系和图纸像素对齐。重复识别会自动沿用同户型历史中对齐更高且无硬错误的墙图，同时合并最新门窗候选，避免随机复跑把已经正确的阶梯墙改坏。人工仍可在原图叠加层拖画墙或矩形房间、移动端点、校准尺度和审核候选；锁定后 Three.js 一次性生成整套可漫游灰模。
- **自动机位 → 多通道约束 → 两轮生成 → 自动 QA**：完整灰模先由本地几何算法生成房间内候选并排除墙外、贴墙和固定物碰撞，Gemini 只能从这些不可改坐标的候选预览中复排构图；入选机位会自动固化 RGB 灰模、线性深度、表面法线、可见边线和语义角色五通道，再同时交给 B2 / Pro 两轮出图。候选预览、评分、AI 理由、五通道和最终任务全部永久留档，人工漫游选机位降为高级纠偏。最终视觉 QA 对照灰模与深度检查机位、墙体、门窗、遮挡和地板范围；网络失败项可串行补评，原评价、错误和每次尝试全部保留。完整设计见 [整屋 3D 链路说明](./docs/WHOLE_HOME_3D_PIPELINE.md)。
...
- 直接读取 DWG / DXF，或从户型图 / PDF 提取一套共享墙体拓扑；CAD 路线保留 handle、图层、块链、位置、旋转和尺寸证据，图片路线仍可人工校准；
- 一次性生成完整整屋 3D 灰模，本地筛选安全候选、Gemini 受限复排并自动保存真实相机；人工漫游保留为特殊构图纠偏；
- 为每个机位固定 RGB、深度、法线和边线缓冲，再按“结构写实化 → 只应用地板”两轮生成；B2 与 Pro 可同时产出候选；
- 对最终候选执行机位、墙体、开口、遮挡和地板范围的自动回归检查；
...
- 当前回归集覆盖提示词黄金样本、路由契约、路径安全、队列恢复、电影规划、整屋统一模型与坐标换算、DWG/DXF 本地转换与许可门禁、CAD 候选选择/动态块/事实哈希、外墙闭环/封闭房间缺墙、本地图纸对齐、三阶段 AI 审计、历史择优、共享墙/开口拓扑、自动机位 AI 受限选择与本地回退、机位版本失效、五类灰模缓冲、本地图像对齐门禁、两轮提示词输入隔离、硬约束复核与失败补评、旧户型人工标注兼容、套图评分、地板占比、校色、独立样品对色、智能选区和记录链路；本次发布为 **389 tests passed**。
...
要求：Python 3.10+、Node.js 20+，并至少配置一个可用图像模型 API——[Google AI Studio](https://aistudio.google.com/)（Gemini）或 [fal.ai](https://fal.ai/) 任一即可；也可连接自备的 ComfyUI 实例，本地算力零 API 费用。CLIPSeg、MobileSAM 与 Depth Anything V2 Small ONNX 模型资产已包含在仓库中，语义地板、精细边界、相对深度审核和最终彩膜投影均可离线运行。
