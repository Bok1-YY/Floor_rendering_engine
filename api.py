...
def auto_correct_floor_color(generated_img, conf_data, strength=0.7):
    """利用置信度分析的 ΔL/ΔC/ΔH，在 LAB 空间对生成图做定向像素级微调。

    只在画面底部（地板区域）施加校正，向上逐渐减弱至零，避免影响墙面和家具。

    Args:
        generated_img: 生成的效果图 (PIL Image, RGB)
        conf_data:     compare_floor_colors() 返回的 dict，含 delta_L/delta_C/delta_H
        strength:      校正强度 0.0~1.0，默认 0.7

    Returns:
        PIL.Image (RGB) 校正后的图，失败时返回原图
    """
    try:
        import numpy as np

        # delta_L 不再用于校正（明度差由室内光造成，应保留），仅校色味
        delta_C = conf_data.get('delta_C', 0)
        delta_H = conf_data.get('delta_H', 0)

        # 微小偏差无需校正（仅看色味：彩度/色相；明度差是室内光造成的，不校正）
        if abs(delta_C) < 2 and abs(delta_H) < 4:
            return generated_img

        # 1. 转 LAB 并创建垂直渐变遮罩（底部=地板=强校正，顶部=零校正）
        lab = generated_img.convert('LAB')
        w, h = lab.size
        lab_arr = np.array(lab, dtype=np.float32)

        mask = np.zeros((h, w), dtype=np.float32)
        floor_line = int(h * 0.65)
        for y in range(floor_line, h):
            mask[y, :] = (y - floor_line) / (h - floor_line)
        mask = mask * strength
        ...
        # 3. PIL LAB → 标准 CIELAB
        L_std = lab_arr[..., 0] * 100.0 / 255.0
        A_std = lab_arr[..., 1] - 128.0
        B_std = lab_arr[..., 2] - 128.0

        # 4. 极坐标
        chroma = np.sqrt(A_std**2 + B_std**2)
        hue = np.degrees(np.arctan2(B_std, A_std)) % 360.0

        # 5. 定向校正（仅活跃像素）——只校色味（彩度/色相），明度保持不变，
        #    保留室内光带来的自然明暗渐变，避免把地板压成均匀一块。
        chroma[active] = np.maximum(chroma[active] - delta_C * mask[active], 0)
        hue[active] = (hue[active] - delta_H * mask[active]) % 360.0

        # 6. 还原直角坐标
        hue_rad = np.radians(hue)
        A_new = chroma * np.cos(hue_rad)
        B_new = chroma * np.sin(hue_rad)

        # 7. 还原 PIL LAB
        L_pil = np.clip(L_std * 255.0 / 100.0, 0, 255)
        A_pil = np.clip(A_new + 128.0, 0, 255)
        B_pil = np.clip(B_new + 128.0, 0, 255)

        corrected_lab = np.stack([L_pil, A_pil, B_pil], axis=2).astype(np.uint8)
        corrected_rgb = np.array(Image.fromarray(corrected_lab, mode='LAB').convert('RGB'), dtype=np.float32)

        # 8. 按遮罩与原图混合（非地板区域原封不动）
        src_rgb = np.array(generated_img.convert('RGB'), dtype=np.float32)
        mask_3ch = np.stack([mask, mask, mask], axis=2)
        blended = src_rgb * (1.0 - mask_3ch) + corrected_rgb * mask_3ch
