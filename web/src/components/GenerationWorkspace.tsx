...
export default function GeneratePage() {
  ...
  useEffect(() => {
    api
      ...
      .then((o) => {
        ...
        if (reuse?.params) {
          setParams(() => hydrateSceneParams({ ...buildDefaultParams(o), ...reuse.params }, o.scene_catalog));
        ...
        setParams((p) => {
          ...
          return hydrateSceneParams(draft?.params ? { ...defaults, ...draft.params } : defaults, o.scene_catalog);
        });
        if (draft) {
  ...
  const updateParams = (patch: Partial<GenParams>) => {
    ...
    const result = applySceneChange(paramsRef.current, patch, options.scene_catalog);
    paramsRef.current = result.params;
    setParams(result.params);
    if (result.corrections.length > 0) {
      toast.info(`为保持场景合理，已联动：${result.corrections.join("；")}`);
    }
  };
  ...
  function applyCustomRecipe(r: CustomRecipe) {
    setParams((p) => options
      ? hydrateSceneParams({ ...p, ...recipeSnapshot(r.params) }, options.scene_catalog)
      : { ...p, ...recipeSnapshot(r.params) });
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <button
              onClick={() =>
                api
                  .clearCompleted()
                  .then((r) => toast.success(`已清除 ${r.cleared} 个已完成任务卡；图片和历史记录均已保留`))
