  ...
  return (
    <div className="flex h-full w-full overflow-hidden text-[14px] text-foreground">
      ...
      <aside className="flex w-[236px] flex-none flex-col border-r border-border bg-panel px-[14px] py-[18px]">
        <div className="flex items-center gap-[10px] px-1.5 pb-5">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[10.5px] font-semibold tracking-wider text-[#9a9082]">
              生图引擎 · 商业版
            </div>
          </div>
        </div>
        ...
        <div className="mt-2 flex items-center gap-[10px] border-t border-border pt-3 pl-1.5">
          <div className="flex h-[30px] w-[30px] items-center justify-center rounded-[9px] bg-[#2e8c7e] text-[13px] font-bold text-white">
            运
          </div>
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <div className="flex h-14 flex-none items-center justify-between border-b border-border px-[22px] backdrop-blur-[8px] bg-[rgba(255,253,249,.72)]">
          <div className="flex flex-col leading-tight">
            <span className="text-[15px] font-bold tracking-tight">{title}</span>
            <span className="text-[11.5px] text-[#9a9082]">{sub}</span>
          </div>
          <button
            onClick={() => setHelpOpen(true)}
            className="flex h-8 items-center gap-1.5 rounded-lg border border-border bg-card px-[13px] text-[12.5px] font-semibold text-[#6b6356] hover:bg-[#f2e9e0]"
          >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="max-w-md">
          <div className="space-y-3 text-sm">
            ...
            <ul className="list-disc space-y-1.5 pl-4 text-[#6b6356]">
              <li>
