...
def build_targeted_cut_review_bundle(document,manifest_path:Path,result_dir:Path,*,_skip_validate=False):
    doc=validate_v21_document(document);manifest_bytes=manifest_path.read_bytes();manifest_hash=hashlib.sha256(manifest_bytes).hexdigest();manifest=json.loads(manifest_bytes.decode('utf-8'));rows=[]
    for oid in ('OP004','OP009'):
        ...
        if result.get('manifest_file_sha256')!=manifest_hash or result.get('image_bindings')!=expected or not result.get('geometry_advisory_complete'):raise ValueError(f'{oid} targeted result binding/incomplete')
