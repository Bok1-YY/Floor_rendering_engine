...
def _save_png_atomic(image,path):
 payload=io.BytesIO();image.save(payload,format='PNG');data=payload.getvalue();tmp=path.with_name(f'.{path.name}.{os.getpid()}.tmp');tmp.write_bytes(data)
