...
def prompt(oid):return f'''Review ONLY {oid}. The red segment is a candidate opening void; purple is the candidate host centerline; blue lines are candidate host wall faces; cyan F-A and orange F-B are distinct pre-cut free-space faces. Judge the underlying floor-plan pixels, not the overlay labels. Return yes only if the visible drawing supports the red segment as an opening/gap on that host and F-A/F-B are visibly on opposite sides. Do not name rooms, infer dimensions, traversal, adjacency, or entrance status. Return one minified JSON object only: {{"opening_id":"{oid}","segment_on_visible_opening":"yes|no|unclear","host_alignment":"agree|conflict|unclear","opposite_face_geometry":"agree|conflict|unclear","visual_kind":"door|window|glazed_interface|wall_gap|unknown|null","confidence":"high|medium|low|null"}}'''
def parse(text,oid):
    ...
    return v
def execute(config_path,manifest_path,oid,output_path):
    import requests
    cfg=json.loads(Path(config_path).read_text(encoding='utf-8'));key=str(cfg.get('fal_api_key') or '').strip();manifest=json.loads(Path(manifest_path).read_text(encoding='utf-8'));row=next(x for x in manifest['openings'] if x['opening_id']==oid);bindings=[];urls=[]
    for role in ('full','crop'):
        p=Path(row['artifacts'][role]['path']);raw=p.read_bytes();urls.append('data:image/png;base64,'+base64.b64encode(raw).decode());bindings.append({'filename':p.name,'bytes':len(raw),'sha256':_sha(raw)})
    text=prompt(oid);payload={'image_urls':urls,'prompt':text,'system_prompt':'Return only the requested JSON. Treat overlays as hypotheses, not facts.','model':MODEL,'reasoning':False,'temperature':0,'max_tokens':256,'enable_web_search':False};proxy=str(cfg.get('fal_queue_proxy') or cfg.get('proxy') or '').strip();resp=None;transport=None
    ...
    result={'schema':'fal-targeted-cut-review-v1','opening_id':oid,'provider':'fal-openrouter-vision','model':MODEL,'http_status':status,'prompt_sha256':_sha(text.encode()),'manifest_file_sha256':_sha(Path(manifest_path).read_bytes()),'image_bindings':bindings,'parsed':parsed,'geometry_advisory_complete':parsed is not None and error is None,'cut_confirmation':False,'pair_confirmation':False,'adjacency_confirmation':False,'semantic_promotion':False,'score_effect':'none','build_authorized':False,'usage':raw.get('usage') if isinstance(raw,dict) else None,'validation_error':error,'transport_error':transport,'raw_response':raw};Path(output_path).parent.mkdir(parents=True,exist_ok=True);Path(output_path).write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8');return result
def main(argv=None):
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--config',type=Path,required=True);p.add_argument('--manifest',type=Path,required=True);p.add_argument('--opening',choices=['OP004','OP009'],required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args(argv);r=execute(a.config,a.manifest,a.opening,a.output);print(json.dumps({k:r[k] for k in ('opening_id','http_status','geometry_advisory_complete','parsed','validation_error')},ensure_ascii=False));return 0 if r['geometry_advisory_complete'] else 2
