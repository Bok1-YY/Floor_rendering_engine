...
def main(argv=None):
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--config',type=Path,required=True);p.add_argument('--manifest',type=Path,required=True);p.add_argument('--opening',choices=['OP002','OP004','OP009','OP010'],required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args(argv);r=execute(a.config,a.manifest,a.opening,a.output);print(json.dumps({k:r[k] for k in ('opening_id','http_status','geometry_advisory_complete','parsed','validation_error')},ensure_ascii=False));return 0 if r['geometry_advisory_complete'] else 2
