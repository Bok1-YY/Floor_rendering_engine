...
def build_partition_targeted_review_bundle(document,manifest_path:Path,result_dir:Path,*,_skip_validate=False):
 doc=validate_v21_document(document);manifest_bytes=manifest_path.read_bytes();manifest_hash=hashlib.sha256(manifest_bytes).hexdigest();manifest=json.loads(manifest_bytes.decode());rows=[];failed=[]
 for oid in ('OP002',):
 ...
 for oid in ('OP006','OP007'):
  geometry=next(x for x in manifest['openings'] if x['opening_id']==oid);p=result_dir/oid/'result.json';raw=p.read_bytes();value=json.loads(raw.decode());expected=[{'filename':Path(geometry['artifact_bindings'][role]['path']).name,'bytes':geometry['artifact_bindings'][role]['bytes'],'sha256':geometry['artifact_bindings'][role]['sha256']} for role in ('full','crop')]
  if value.get('manifest_file_sha256')!=manifest_hash or value.get('image_bindings')!=expected or value.get('geometry_advisory_complete') is not False or value.get('parsed') is not None:raise ValueError(f'{oid} failed review binding/state drift')
  failed.append({'opening_id':oid,'result_file_sha256':hashlib.sha256(raw).hexdigest(),'result_canonical_sha256':_hash(value),'cost_usd':float((value.get('usage') or {}).get('cost') or 0),'reason':'bare_json_protocol_failed','parsed':None,'image_bindings':expected})
 result={'schema':'partition-targeted-review-bundle-v3','source_structure_hash':doc['structure_hash'],'evidence_file_sha256':manifest_hash,'evidence_candidate_hash':manifest['candidate_hash'],'covered_opening_ids':['OP002'],'failed_review_attempt_ids':['OP006','OP007'],'excluded_opening_reviews':[{'opening_id':'OP010','reason':'prior flash and pro reviews violated bare-JSON protocol; unchanged visual method not repeated after policy-only migration'}],'reviews':rows,'failed_reviews':failed,'usable_count':sum(r['usable_advisory'] for r in rows),'total_cost_usd':round(sum(r['cost_usd'] for r in rows)+sum(r['cost_usd'] for r in failed),10),'correction_review_candidate_ids':[r['opening_id'] for r in rows if r['visual_geometry_agreement']],'cut_confirmation':False,'pair_confirmation':False,'adjacency_confirmation':False,'semantic_promotion':False,'score_effect':'none','build_authorized':False,'ready':False,'candidate_hash':'0'*64};result['candidate_hash']=_hash({k:v for k,v in result.items() if k!='candidate_hash'})
...
def validate_partition_targeted_review_bundle(document,candidate):
 ...
 if candidate.get('schema')!='partition-targeted-review-bundle-v3' or candidate.get('source_structure_hash')!=doc['structure_hash'] or candidate.get('covered_opening_ids')!=['OP002'] or candidate.get('failed_review_attempt_ids')!=['OP006','OP007']:raise ValueError('partition targeted bundle source/coverage drift')
