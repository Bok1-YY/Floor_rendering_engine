...
def build_partition_targeted_review_bundle(document,manifest_path:Path,result_dir:Path,*,_skip_validate=False):
 doc=validate_v21_document(document);manifest=json.loads(manifest_path.read_text());rows=[]
 for oid in ('OP002','OP010'):
  ...
  if result.get('image_bindings')!=expected:raise ValueError(f'{oid} partition targeted image drift')
 ...
 result={'schema':'partition-targeted-review-bundle-v1','source_structure_hash':doc['structure_hash'],'evidence_file_sha256':hashlib.sha256(manifest_path.read_bytes()).hexdigest(),'evidence_candidate_hash':manifest['candidate_hash'],'covered_opening_ids':['OP002','OP010'],'reviews':rows,'usable_count':sum(r['usable_advisory'] for r in rows),'total_cost_usd':round(sum(r['cost_usd'] for r in rows),10),'correction_review_candidate_ids':[r['opening_id'] for r in rows if r['visual_geometry_agreement']],'cut_confirmation':False,'pair_confirmation':False,'adjacency_confirmation':False,'semantic_promotion':False,'score_effect':'none','build_authorized':False,'ready':False,'candidate_hash':'0'*64};result['candidate_hash']=_hash({k:v for k,v in result.items() if k!='candidate_hash'})
...
def validate_partition_targeted_review_bundle(document,candidate):
 ...
 if candidate.get('schema')!='partition-targeted-review-bundle-v1' or candidate.get('source_structure_hash')!=doc['structure_hash'] or candidate.get('covered_opening_ids')!=['OP002','OP010']:raise ValueError('partition targeted bundle source/coverage drift')
