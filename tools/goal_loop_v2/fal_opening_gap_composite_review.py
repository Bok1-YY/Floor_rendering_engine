"""Strict cross-image XY validity review for an isolated Blender gap variant."""
...
from tools.goal_loop_v2.build_opening_gap_composites import validate as validate_composites
...
FIELDS = (
    "opening_id",
    "source_segment_visible",
    "model_gap_centered_on_source_segment",
    "model_gap_width_matches_source_xy",
    "junction_or_neighbor_obstruction",
    "xy_variant_visually_valid",
    "recommendation",
    "confidence",
)
...
def parse(text: str, opening_id: str) -> dict[str, Any]:
    if not isinstance(text, str) or not text.strip().startswith("{") or not text.strip().endswith("}"):
        raise ValueError("gap composite review must be bare JSON")
    value = json.loads(text.strip())
    if set(value) != set(FIELDS) or value.get("opening_id") != opening_id:
        raise ValueError("gap composite review schema/id mismatch")
    if any(value[key] not in {"yes", "no", "unclear"} for key in FIELDS[1:6]):
        raise ValueError("gap composite review enum mismatch")
    if value["recommendation"] not in {"accept_xy_variant", "reject_xy_variant", "unclear"} or value["confidence"] not in {"high", "medium", "low", None}:
        raise ValueError("gap composite review enum mismatch")
    return value
...
def execute(config_path: str | Path, evidence_path: str | Path, output_path: str | Path, opening_id: str, model: str = MODEL) -> dict[str, Any]:
    ...
    validate_composites(evidence, rebuild=False)
    try:
        row = next(item for item in evidence["rows"] if item["opening_id"] == opening_id)
    except StopIteration as exc:
        raise ValueError(f"opening composite unavailable: {opening_id}") from exc
    artifact_order = ("composite", "source_crop", "model_closeup")
    bindings, images = [], []
    for role in artifact_order:
        ...
        if len(raw) != artifact["bytes"] or _sha(raw) != artifact["sha256"]:
            raise ValueError("gap composite review artifact drift")
        bindings.append({"role": role, "filename": path.name, "bytes": len(raw), "sha256": artifact["sha256"]})
        images.append({"type": "image_url", "image_url": {"url": "data:image/png;base64," + base64.b64encode(raw).decode("ascii")}})

    prompt = (
        f"Compare ONLY the XY placement for {opening_id}. Image 1 is the side-by-side evidence, Image 2 is the untouched source crop, "
        f"and Image 3 is the Blender close-up. The source local segment is {json.dumps(row['source_local_segment_px'])}; the projected "
        f"gap width is {row['projected_gap_width_m']:.6f} m. Decide whether the model gap is centered on and has the same XY extent as "
        "the visible source segment, and whether a junction or neighboring wall obstructs it. Full height is display-only. Do not infer "
        "door/window type, rooms, Z/head/sill, traversability, adjacency, source correction, or construction. Return only strict JSON."
    )
    system_prompt = "Return only the requested JSON object. Source and model images are separate evidence roles."
    ...
    schema = {
        ...
        "properties": {
            ...
            **{key: categorical for key in FIELDS[1:6]},
            "recommendation": {"type": "string", "enum": ["accept_xy_variant", "reject_xy_variant", "unclear"]},
    ...
    body = {
        ...
        "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": [{"type": "text", "text": prompt}, *images]}],
        "response_format": {"type": "json_schema", "json_schema": {"name": "opening_gap_xy_review", "strict": True, "schema": schema}},
        "temperature": 0,
        "max_tokens": 256,
    }
    contract = {"model": selected_model, "prompt_sha256": _sha(prompt.encode()), "system_sha256": _sha(system_prompt.encode()), "image_bindings": bindings, "response_format": body["response_format"], "temperature": 0, "max_tokens": 256}
    ...
    validation_error, parsed = transport_error, None
    ...
    result = {
        "schema": "fal-opening-gap-composite-review-v2",
