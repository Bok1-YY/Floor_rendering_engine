"""Generic fail-closed Gemini visual-subtype advisory for a clean opening crop."""
...
def parse(text: str, opening_id: str) -> dict[str, Any]:
    if opening_id not in EXPECTED_INCLUDED:
        raise ValueError("opening is not admitted to the clean subtype research set")
    if not isinstance(text, str) or not text.strip().startswith("{") or not text.strip().endswith("}"):
        raise ValueError("clean subtype review must be bare JSON")
    value = json.loads(text.strip())
    if set(value) != set(FIELDS) or value.get("opening_id") != opening_id:
        raise ValueError("clean subtype review schema/id mismatch")
    if value.get("visual_kind") not in VISUAL_KINDS:
        raise ValueError("clean subtype review visual-kind enum mismatch")
    if any(value.get(key) not in {"yes", "no", "unclear"} for key in FIELDS[2:5]):
        raise ValueError("clean subtype review cue enum mismatch")
    if value.get("confidence") not in {"high", "medium", "low", None}:
        raise ValueError("clean subtype review confidence mismatch")
    return value
...
def execute(
    ...
    try:
        ...
        raise ValueError(f"{opening_id} clean evidence is unavailable") from exc
    if (
        row.get("source_pixels_untouched") is not True
        or row.get("authority") not in {"source_active", "registered_evidence_candidate"}
        or row.get("matrix_cuttable") is not True
    ):
        raise ValueError(f"{opening_id} clean evidence authority drift")
    ...
    prompt = (
        f"Review ONLY {opening_id} as a visual-subtype candidate. Image 1 is the byte-exact clean raw crop and is "
        "the ONLY semantic image authority. Image 2 is a locator for navigation only; its colored box/line/marks "
        "must not be used as subtype evidence. Classify visual_kind only from source-pixel cues visible in image 1. "
        "Report whether a wall break, door swing arc, or sliding-track cue is visibly present. This is a visual "
        "advisory, not source confirmation. Do NOT infer head, sill, Z, rooms, room pairs, traversability, adjacency, "
    ...
    response_format = {
        "type": "json_schema",
        "json_schema": {
            "name": f"{opening_id.lower()}_clean_visual_subtype",
            "strict": True,
            "schema": schema,
        },
    }
    ...
    result = {
        "schema": "fal-clean-subtype-review-v1",
        "opening_id": opening_id,
        "endpoint": ENDPOINT,
        "model": selected_model,
