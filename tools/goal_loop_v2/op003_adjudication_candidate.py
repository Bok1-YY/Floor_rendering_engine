...
PAIR=["bedroom_01","west_toilet"]; JAMB_MIN_M=.12
...
def _support(segment,host):
    ...
    return {'host_atom_id':HOST_ID,'endpoint_support':[{'endpoint_index':i,'segment_endpoint_m':list(p),'host_parameter':round(vals[i],9),'supported':0.0<=vals[i]<=1.0} for i,p in enumerate((a,b))],'support_length_m':round(math.dist(a,b),9),'host_start_jamb_m':round(start_jamb,9),'host_start_jamb_min_m':JAMB_MIN_M,'host_start_jamb_sufficient':start_jamb>=JAMB_MIN_M,'jamb_insufficient':start_jamb<JAMB_MIN_M,'derivation':'segment endpoints projected onto host centerline; start jamb measured to host start'}
