...
def host_geometry(atom,segment,minimum):
 ...
 return faces,{'host_parameters':[round(v,9) for v in params],'jamb_before_m':round(before,9),'jamb_after_m':round(after,9),'minimum_jamb_m':round(min(before,after),9),'candidate_minimum_m':minimum,'candidate_sufficient':min(before,after)>=minimum,'policy_source':'opening_contract.minimum_jamb_support_m'}
def draw_geometry(image,box,inv,pts,host,faces,minimum):
 ...
 host_faces,_=host_geometry(host,pts,minimum)
...
def main():
 ...
 for x in impacts:
  ...
  host_faces,jamb=host_geometry(host,seg,minimum)
  pts=[tuple(px[0]),tuple(px[1])]; allxy=pts
  for f in faces:
   allxy += [tuple(_apply(_inverse(m),q)) for q in f['polygon']]
  pad=120; box=(max(0,int(min(q[0] for q in allxy)-pad)),max(0,int(min(q[1] for q in allxy)-pad)),min(im.width,int(max(q[0] for q in allxy)+pad)),min(im.height,int(max(q[1] for q in allxy)+pad)));crop=im.crop(box);inv=_inverse(m);draw_geometry(crop,box,inv,seg,host,faces,minimum)
  cp=OUT/f'{oid}-crop.png';crop.save(cp);full=im.copy();draw_geometry(full,(0,0,im.width,im.height),inv,seg,host,faces,minimum);fp=OUT/f'{oid}-full.png';full.save(fp);rows.append({'opening_id':oid,'segment_m':seg,'registration':reg,'host_atom_id':host['id'],'host_centerline_m':host['centerline_m'],'host_thickness_m':host['thickness_m'],'host_face_segments_m':host_faces,'jamb_support':jamb,'merged_group':merged,'pre_cut_faces':faces,'post_cut_anchor_groups':x['post_cut_anchor_groups'],'sensitivity_stable':all(s['merged_groups']==x['merged_groups'] for s in x['sensitivity']) and all(s['merged_groups']==x['merged_groups'] for s in x['endpoint_sensitivity']),'artifacts':{'crop':{'path':str(cp.resolve()),'sha256':sha(cp)},'full':{'path':str(fp.resolve()),'sha256':sha(fp)}},'cut_confirmation':False,'pair_confirmation':False,'adjacency_confirmation':False,'build_authorized':False})
