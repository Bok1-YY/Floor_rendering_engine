...
def host_geometry(atom,segment):
 a,b=atom['centerline_m'];dx,dy=b[0]-a[0],b[1]-a[1];length=math.hypot(dx,dy);nx,ny=-dy/length,dx/length;half=atom['thickness_m']/2;faces=[[[a[0]+nx*half,a[1]+ny*half],[b[0]+nx*half,b[1]+ny*half]],[[a[0]-nx*half,a[1]-ny*half],[b[0]-nx*half,b[1]-ny*half]]];params=[((p[0]-a[0])*dx+(p[1]-a[1])*dy)/(length*length) for p in segment];lo,hi=min(params),max(params);before=max(0,lo*length);after=max(0,(1-hi)*length);minimum=min(before,after)
 return faces,{'host_parameters':[round(v,9) for v in params],'jamb_before_m':round(before,9),'jamb_after_m':round(after,9),'minimum_jamb_m':round(minimum,9),'threshold_m':.12,'sufficient':minimum>=.12}
...
def draw(image,box,inv,segment,atom,public_geom,nonpublic):
 dr=ImageDraw.Draw(image);offset=(box[0],box[1]);px=lambda p:(lambda q:(q[0]-offset[0],q[1]-offset[1]))(_apply(inv,p));dr.line([px(p) for p in atom['centerline_m']],fill=(160,40,220),width=5);host_faces,_=host_geometry(atom,segment)
...
def build():
 ...
 for oid in IDS:
  ...
  px_segment=[list(_apply(inv,p)) for p in segment];registration=validate_pixel_metric_segment(metric,px_segment,segment,1.0);host_faces,jamb=host_geometry(atom,segment);all_metric=[*segment,*atom['centerline_m'],*[q for p in _polygon_parts(public_geom) for q in p.exterior.coords],*nonpublic.exterior.coords];all_px=[_apply(inv,p) for p in all_metric];pad=100;box=(max(0,int(min(p[0] for p in all_px)-pad)),max(0,int(min(p[1] for p in all_px)-pad)),min(source_image.width,int(max(p[0] for p in all_px)+pad)),min(source_image.height,int(max(p[1] for p in all_px)+pad)));crop=source_image.crop(box);draw(crop,box,inv,segment,atom,public_geom,nonpublic);full=source_image.copy();draw(full,(0,0,source_image.width,source_image.height),inv,segment,atom,public_geom,nonpublic);cp=OUT/f'{oid}-crop.png';fp=OUT/f'{oid}-full.png';crop.save(cp);full.save(fp)
...
def main():
 result=build();OUT.mkdir(parents=True,exist_ok=True);(OUT/'partition-targeted-adjudication.json').write_text(json.dumps(result,indent=2)+'\n');(OUT/'REPORT.md').write_text('# Partition-targeted adjudication evidence v2\n\nS-P is the research semantic public cell; F-N is the distinct physical non-public pre-cut face. OP002/OP010 have sufficient same-wall jamb support; OP006/OP007 remain insufficient. Candidate-only; no confirmation or promotion.\n');print(result['candidate_hash'])
